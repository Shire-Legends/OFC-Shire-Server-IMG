...
void Creature::onThink(uint32_t interval)
{
	...
	checkConditionsEvents(interval);
...
void Creature::onWalkFear()
{
	if (hasCondition(CONDITION_FEAR)) {
		...
		if (r <= 4) {
			switch (r) {
			case 0: dir = DIRECTION_NORTH; break;
			case 1: dir = DIRECTION_WEST;  break;
			case 3: dir = DIRECTION_SOUTH; break;
			case 4: dir = DIRECTION_EAST;  break;
			default:
				break;
			}

			if (hasCondition(CONDITION_FEAR)) {
				g_game.internalCreatureSay(this, TALKTYPE_MONSTER_SAY, "Socorro!", false);
...
void Creature::onAddCondition(ConditionType_t type)
{
	...
	}else if (type == CONDITION_FEAR && hasCondition(CONDITION_HASTE)) {
		removeCondition(CONDITION_HASTE);
	}else if (type == CONDITION_HASTE && hasCondition(CONDITION_FEAR)) {
		removeCondition(CONDITION_HASTE);
	}
}
...
void Creature::onGainExperience(uint64_t gainExp, Creature* target)
{
	...
	if (getPlayer() && getPlayer()->getLevel() >= 500) {
		return;
	}
...
void Creature::checkConditionsEvents(uint32_t interval)
{
	if (hasCondition(CONDITION_FEAR)) {
		onWalkFear();
	}
