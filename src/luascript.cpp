...
void LuaScriptInterface::registerFunctions()
{
	...
	//getDepotId(uid)
	lua_register(luaState, "getPlayerReset", LuaScriptInterface::luaGetPlayerReset);
	lua_register(luaState, "setPlayerReset", LuaScriptInterface::luaSetPlayerReset);
	lua_register(luaState, "doResetLevel", LuaScriptInterface::luaDoResetLevel);
	...
	registerEnum(CONDITION_WEAKNESS)
	...
	registerEnum(CONDITION_ARMOR_UP)
	...
	registerEnum(CONDITION_EMPTY)
	registerEnum(CONDITION_IMMORTAL)
	registerEnum(CONDITION_FEAR)
	registerEnum(CONDITION_ROOTS)
	registerEnum(CONDITION_DOUBLE_XP)
	...
	registerEnum(CONST_ME_NEWFIREHIT)
	...
	registerEnum(CONST_ME_NEWENERGYHIT)
	...
	registerEnum(CONST_ANI_BIGFIRE)
	...
	registerEnum(CONST_ANI_SHADOWBOLT)
	registerEnum(CONST_ANI_PHANTOMBOLT)
	...
	registerEnum(CONST_ANI_BULLET)
	registerEnum(CONST_ANI_SMALLICE)
	registerEnum(CONST_ANI_BIGEARTH)
	registerEnum(CONST_ANI_ELEMENTALBOLT)
	...
	registerEnum(CONST_ANI_BIGENERGY)
	...
	registerEnum(RETURNVALUE_NOTATTACKTHISPLAYERGUILD)		
	...
	registerMethod("Item", "getTypeI", LuaScriptInterface::luaItemGetType);
	...
	registerMethod("Player", "kickPlayer", LuaScriptInterface::luaPlayerKickPlayer);
	...
	registerMethod("Player", "addMaxHealth", LuaScriptInterface::luaPlayerAddMaxHealth);
	...
	registerMethod("Player", "addSkillLevel", LuaScriptInterface::luaPlayerAddSkillLevel);
...
int LuaScriptInterface::luaPlayerKickPlayer(lua_State* L)
{
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerAddMaxHealth(lua_State* L)
{
	Player* player = getUserdata<Player>(L, 1);
	int32_t value = getNumber<int32_t>(L, 2);

	if (player) {
		player->healthMax = value;
		player->health = value;
...
int LuaScriptInterface::luaPlayerAddSkillLevel(lua_State* L)
{
	// player:addSkillTries(skillType, tries)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		skills_t skillType = getNumber<skills_t>(L, 2);
		uint64_t level = getNumber<uint64_t>(L, 3);
		player->setAddSkillLevel(skillType, level);
...
int32_t LuaScriptInterface::luaGetPlayerReset(lua_State* L)
{
	//getPlayerReset(cid)
	Player* player = getUserdata<Player>(L, 1);
	if (player)
		lua_pushnumber(L, player->getPlayerReset());
	else
	{
		lua_pushnil(L);
	}
...
int32_t LuaScriptInterface::luaSetPlayerReset(lua_State* L)
{
	//setPlayerReset(cid, amount)
	int32_t amount = getNumber<int32_t>(L,2);
	Player* player = getUserdata<Player>(L, 1);
	if (player)
	{
		player->setPlayerReset(amount);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaDoResetLevel(lua_State* L)
{
	Player* player = getUserdata<Player>(L, 1);
	if (player)
	{
...
int LuaScriptInterface::luaItemTypeGetAbilities(lua_State* L)
{
	...
	if (itemType) {
		...
		setField(L, "dodgePercent", abilities.dodgePercent);
