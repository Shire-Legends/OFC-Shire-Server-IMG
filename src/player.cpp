...
float Player::getDefenseFactor() const
{
	...
	case FIGHTMODE_BALANCED:
		return (OTSYS_TIME() - lastAttack) < getAttackSpeed() ? 1.0f : 1.25f;
	case FIGHTMODE_DEFENSE:
		return 1.5f;
	default:
		return 1.0f;
							 ...
							 bool checkDefense /* = false*/, bool checkArmor /* = false*/, bool field /* = false*/, bool ignoreResistances /* = false*/)
{
	...
	for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; ++slot)
	{
		...
		if (absorbPercent != 0)
		{
			...
			{
				damage = (int32_t)std::ceil(damage * (100 - (absorbPercent * 1.3)) / 100);
			}
			else if (hasCondition(CONDITION_WEAKNESS))
			{
				damage = (int32_t)std::ceil(damage * (100 - (absorbPercent * 0.7)) / 100);
			}
			else
			{
