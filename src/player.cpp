...
uint32_t Player::getAttackSpeed() const
{
	...
	Condition* condition = getCondition(CONDITION_HASTEGA, CONDITIONID_DEFAULT);
...
float Player::getAttackFactor() const
{
	...
	case FIGHTMODE_BALANCED: return 2.1f;
	case FIGHTMODE_DEFENSE: return 3.0f;
	default: return 1.0f;
	}
}
...
float Player::getDefenseFactor() const
{
	...
	case FIGHTMODE_BALANCED: return (OTSYS_TIME() - lastAttack) < getAttackSpeed() ? 1.0f : 1.25f;
	case FIGHTMODE_DEFENSE: return 1.5f;
	default: return 1.0f;
	}
}
...
void Player::sendStats()
{
	...
	if (experience >= 2058391381)
	{
		experience = 2058391381;
	}
}
...
void Player::onCreatureAppear(Creature* creature, bool isLogin)
{
	...
	if (isLogin && creature == this) {
		...
		g_game.addAnimatedText("Online!", this->getPosition(), TEXTCOLOR_BLUE);
	...
	bool checkDefense /* = false*/, bool checkArmor /* = false*/, bool field /* = false*/, bool ignoreResistances /* = false*/)
{
	...
	for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; ++slot) {
		...
		if (it.abilities->dodgePercent != 0) {
			if (random_range(0,100) < it.abilities->dodgePercent && getVocationId() > 9) {
				damage = 0;
				g_game.addAnimatedText("Dodge!",getPosition(),TEXTCOLOR_LIGHTBLUE);
				g_game.addMagicEffect(getPosition(), BLOCK_ARMOR);
			}
		}

		const int16_t& absorbPercent = it.abilities->absorbPercent[combatIndex];
		if (absorbPercent != 0) {
			if (hasCondition(CONDITION_ARMOR_UP) && !hasCondition(CONDITION_WEAKNESS)) {
				damage = (int32_t)std::ceil(damage * ((float)(80 - absorbPercent) / 100));
			}
			else if (hasCondition(CONDITION_WEAKNESS)) {
				damage = (int32_t)std::ceil(damage * ((float)(130 - absorbPercent) / 100));
			}
			else {
				damage = (int32_t)std::ceil(damage * ((float)(100 - absorbPercent) / 100));
			}
		...
		if (field) {
			...
			if (fieldAbsorbPercent != 0) {
				...
				damage = (int32_t)std::ceil(damage * ((float)(100 - fieldAbsorbPercent) / 100));
			}
...
void Player::onAddCombatCondition(ConditionType_t type)
{
	...
	case CONDITION_WEAKNESS:
		sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "sua defesa foi reduzida.");
		break;

	case CONDITION_ARMOR_UP:
		sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "sua defesa foi aumentada.");
		break;

	case CONDITION_ROOTS:
		sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "Você está imobilizado.");
		break;

	case CONDITION_HASTEGA:
		sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "Modo RAGE ativado!");
		break;
