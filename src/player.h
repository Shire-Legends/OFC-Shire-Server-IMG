...
class Player final : public Creature, public Cylinder
{
	public:
		...
		void setMaxHealth(uint32_t value) {
			healthMax = value;
		}
	...
	private:
		...
		uint32_t getPlayerReset() {
			return reset;
		}

		void setPlayerReset(int32_t amount) { reset += amount; if (reset < 0) { reset = 0; } }

		void setAddSkillLevel(skills_t type, int32_t level) { skills[type].level += level; skills[type].tries = 0; };
