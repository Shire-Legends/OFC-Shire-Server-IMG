...
int32_t WeaponMelee::getWeaponDamage(const Player* player, const Creature*, const Item* item, bool maxDamage /*= false*/) const
{
	...
	int32_t maxValue = static_cast<int32_t>(Weapons::getMaxWeaponDamageForm( attackSkill, attackValue, attackFactor) * player->getVocation()->meleeDamageMultiplier);
	int32_t minValue = static_cast<int32_t>(Weapons::getMinWeaponDamageForm( attackSkill, attackValue, attackFactor) * player->getVocation()->meleeDamageMultiplier);
...
int32_t WeaponDistance::getWeaponDamage(const Player* player, const Creature* target, const Item* item, bool maxDamage /*= false*/) const
{
	...
	int32_t maxValue = static_cast<int32_t>(Weapons::getMaxWeaponAmmoDamage( attackSkill, attackValue, attackFactor) * player->getVocation()->distDamageMultiplier);
	...
	int32_t minValue;
	if (target) {
		minValue = static_cast<int32_t>(std::ceil(Weapons::getMinWeaponAmmoDamage(attackSkill,attackValue,attackFactor)));
	} else {
		minValue = 0;
	}
	return -uniform_random(minValue, maxValue);
