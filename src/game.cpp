...
bool Game::combatBlockHit(CombatDamage& damage, Creature* attacker, Creature* target, bool checkDefense, bool checkArmor, bool field, bool ignoreResistances /*= false */)
{
	...
	static const auto sendBlockEffect = [this](BlockType_t blockType, CombatType_t combatType, const Position& targetPos) {
		if (blockType == BLOCK_DEFENSE) {
			...
			g_game.addAnimatedText("Dodge!", targetPos, TEXTCOLOR_LIGHTBLUE);
			addMagicEffect(targetPos, CONST_ME_POFF);
		}
		};
...
void Game::combatGetTypeInfo(CombatType_t combatType, Creature* target, TextColor_t& color, uint8_t& effect)
{
	...
	case COMBAT_FIREDAMAGE: {
		...
		effect = CONST_ME_NEWFIREHIT;
		break;
	}
	case COMBAT_ICEDAMAGE: {
		color = TEXTCOLOR_SKYBLUE;
		effect = CONST_ME_BIGICE;
		break;
	}
	case COMBAT_HOLYDAMAGE: {
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		g_game.addAnimatedText(fmt::format("+{:d}", realHealthChange), targetPos, TEXTCOLOR_LIGHTGREEN);
		...
		if (target->hasCondition(CONDITION_IMMORTAL) || Combat::canDoCombat(attacker, target) != RETURNVALUE_NOERROR) {
			g_game.addAnimatedText("Imortal", target->getPosition(), TEXTCOLOR_WHITE_EXP);
			return false;
		}
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (manaChange > 0) {
		...
		g_game.addAnimatedText("+" + std::to_string(realManaChange), target->getPosition(), TEXTCOLOR_ELECTRICPURPLE);

		if (realManaChange > 0 && !targetPlayer->isInGhostMode()) {
			TextMessage message(MESSAGE_EVENT_DEFAULT, "You gained " + std::to_string(realManaChange) + " mana.");
