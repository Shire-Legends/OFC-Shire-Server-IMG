	...
	if (formulaType == COMBAT_FORMULA_DAMAGE) {
		...
		if (creature->getCombatValues(min, max)) {
			...
			if (params.valueCallback) {
				...
				if (weapon) {
					damage.primary.value = normal_random(std::fma(weapon->getWeaponDamage(player, target, tool, true), mina, minb), std::fma(weapon->getWeaponDamage(player, target, tool, true), maxa, maxb));
...
ReturnValue Combat::canDoCombat(Creature* attacker, Creature* target)
{
	...
	if (const Player* targetPlayer = target->getPlayer()) {
		...
		if (const Player* attackerPlayer = attacker->getPlayer()) {
			...
			if (attackerPlayer->getGuild() && targetPlayer->getGuild()) {
				if (attackerPlayer->getGuild()->getId() == attackerPlayer->getGuild()->getId()) {
					return RETURNVALUE_NOTATTACKTHISPLAYERGUILD;
				}
			}
