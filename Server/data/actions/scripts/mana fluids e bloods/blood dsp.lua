...
doPlayerSendTextMessage(player,20,'Desculpe, voc� n�o tem Souls suficiente.')
return true
end

if(os.time() < getPlayerStorageValue(player, exhausted_storagevalue)) then
doPlayerSendTextMessage(player,20,'Voc� n�o pode usar um blood of gods durante o efeito de outro.')
return true
end

if(os.time() < getPlayerStorageValue(player, exhausted_storagevalue2)) then
 doPlayerSendCancel(player,"Voc� est� muito cansado.")
return true
end
...
doPlayerSendCancel(player,"Voc� est� esgotado.")
return true
end
...
doPlayerSendTextMessage(player,20,"Voc� tomou o sangue dos deuses e se transformou em um Chronos.(Boost: regenera��o ao uso da SD.)")
...
player:getPosition():sendMagicEffect(60)
doPlayerSendTextMessage(player,20,"Voc� tomou o sangue dos deuses e se transformou em um Kazard. (Boost: super velocidade e silence ao uso da SD.)")
...
player:getPosition():sendMagicEffect(60)
doPlayerSendTextMessage(player,20,"Voc� tomou o sangue dos deuses e se transformou em um Hazus. (Boost: reflex�o ao uso da SD.)")
doCreatureSay(player, "Minhas m�os est�o queimando!", TALKTYPE_ORANGE_1)
addEvent(Blood1, 1*2000, player:getId())
setPlayerStorageValue(player, exhausted_storagevalue, os.time() + exhausted_seconds)
doPlayerAddSoul(player,-50)
doRemoveItem(item.uid,1)
...
player:getPosition():sendMagicEffect(60)
doPlayerSendTextMessage(player,20,"Voc� tomou o sangue dos deuses e se transformou no Rei. (Boost: imortalidade.)")
doCreatureSay(player, "Haha, o rei est� no comando!", TALKTYPE_ORANGE_1)
addEvent(Blood1, 1*2000, player:getId())
setPlayerStorageValue(player, exhausted_storagevalue, os.time() + exhausted_seconds)
doPlayerAddSoul(player,-50)
doRemoveItem(item.uid,1)
