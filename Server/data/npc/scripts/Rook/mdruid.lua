...
local function creatureSayCallback(cid, type, msg)
	...
	elseif npcHandler.topic[cid] == topicList.MISSIONTEST then
		if msgcontains(msg,"yes") then
			...
			player:setStorageValue(questNumber.QuestTreinamento, 8)
			doPlayerSendTextMessage(cid,23,"Você foi teleportado para o Jardim.")
			doTeleportThing(cid, {x = 264, y = 179, z = 8})
		end
		return true
	end
end

npcHandler:setMessage(MESSAGE_GREET, "Estava te esperando |PLAYERNAME| , ! Tem certeza, quer treinar para ser um Druida Puro? Então diga 'test' mortal.")
