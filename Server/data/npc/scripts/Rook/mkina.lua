...
local function creatureSayCallback(cid, type, msg)
	...
	elseif npcHandler.topic[cid] == topicList.MISSIONTEST then
		if msgcontains(msg,"yes") then
			...
			player:setStorageValue(questNumber.QuestTreinamento, 7)
			doPlayerSendTextMessage(cid,23,"Você foi teleportado para a Sala de Armamento.")
			doTeleportThing(cid, {x = 290, y = 177, z = 8})
		end
		return true
	end
end

npcHandler:setMessage(MESSAGE_GREET, "Estava te esperando |PLAYERNAME| , ! Tem certeza, quer treinar para ser um Cavaleiro Heroico? Então diga 'test' mortal.")
