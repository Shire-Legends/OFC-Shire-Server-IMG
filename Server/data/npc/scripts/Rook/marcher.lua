...
local function creatureSayCallback(cid, type, msg)
	...
	elseif npcHandler.topic[cid] == topicList.MISSIONTEST then
		if msgcontains(msg,"yes") then
			...
			player:setStorageValue(questNumber.QuestTreinamento, 9)
			doPlayerSendTextMessage(cid,23,"Você foi teleportado para o Acampamento dos Minotauros.")
			doTeleportThing(cid, {x = 273, y = 186, z = 8})
		end
		return true
	end
end

npcHandler:setMessage(MESSAGE_GREET, "Estava te esperando |PLAYERNAME| , ! Tem certeza que quer treinar para ser um Arqueiro Preciso? Então diga 'test' mortal.")
