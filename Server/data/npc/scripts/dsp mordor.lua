local itens = {
	...
	darkexplo_id_valor_qtd = {2314,50, 2000},
	darksd_id_valor_qtd = {2267,50, 1000},
...
function creatureSayCallback(cid, type, msg)
    ...
    local talkUser = NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT and 0 or cid
		...
		elseif isInArray(t.dsp, msg) then
			...
			talk_state = 16
		elseif isInArray(t.darkexplo, msg) then
			selfSay('Deseja comprar '..itens.darkexplo_id_valor_qtd[3]..' dark explosions por '.. itens.darkexplo_id_valor_qtd[2].." SLP's?",cid)
			talk_state = 17
		elseif isInArray(t.darksd, msg) then
			selfSay('Deseja comprar '..itens.darksd_id_valor_qtd[3]..' dark sds por '.. itens.darksd_id_valor_qtd[2].." SLP's?",cid)
			talk_state = 18
...
handleDarkSd = function(cid)
	if doPlayerRemoveItem(cid,6527, itens.darksd_id_valor_qtd[2]) then
		dsp = getPlayerItemCount(cid, 6527)
		selfSay('Aqui esta! Obrigado e volte sempre.',cid)
		doPlayerSendTextMessage(cid,22,"Voce recebeu "..itens.darksd_id_valor_qtd[3].." dark sds.")
		doPlayerSendTextMessage(cid, 20, 'Ainda lhe resta ' .. dsp .. ' Shire Legends Points.')
		container = doPlayerAddItem(cid, 2003, 1)
			doAddContainerItem(container, itens.darksd_id_valor_qtd[1], itens.darksd_id_valor_qtd[3])
...
handleDarkExplosion = function(cid)
	if doPlayerRemoveItem(cid,6527, itens.darkexplo_id_valor_qtd[2]) then
		dsp = getPlayerItemCount(cid, 6527)
		selfSay('Aqui esta! Obrigado e volte sempre.',cid)
	doPlayerSendTextMessage(cid,22,"Voce recebeu "..itens.darkexplo_id_valor_qtd[3].." dark explosions.")
	doPlayerSendTextMessage(cid, 20, 'Ainda lhe resta ' .. dsp .. ' Shire Legends Points.')
		container = doPlayerAddItem(cid, 2001, 1)
			doAddContainerItem(container, itens.darkexplo_id_valor_qtd[1], itens.darkexplo_id_valor_qtd[3])
		selfSay('Desculpe, mas você não tem pontos suficiente.')
	end
end
...
handleBpSd = function(cid)
	if doPlayerRemoveItem(cid,6527, itens.bpSd_id_valor_qtd[2]) then
		...
		doPlayerSendTextMessage(cid, 20, 'Ainda lhe resta ' .. dsp .. ' Shire Legends Points.')
		container = doPlayerAddItem(cid, 2003, 1)
			doAddContainerItem(container, itens.bpSd_id_valor_qtd[1], itens.bpSd_id_valor_qtd[3])
