...
function setPlayerMount(cid, mountId) local p = Player(cid) return p ~= nil and p:addMount(mountId) or false end
...
function doCreateItemAction(itemid, count, pos,action)
	local tile = Tile(pos)
	if not tile then
		return false
	end
